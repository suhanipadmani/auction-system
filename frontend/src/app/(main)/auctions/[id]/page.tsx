...
// Hooks
import { useAuctionDetails, useCancelAuction, useAdminApprove, useFinalizeAuction } from "@/hooks/useAuction";
...
export default function AuctionDetailPage() {
   ...
   // Real-time updates
   const { highestBid, highestBidderId, highestBidderName, isPending: isSocketPending, placeBid } = useAuctionSocket(id as string);
      ...
      <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-12">
         <div className="flex items-center justify-between">
            ...
            {isOwner && auction.status === 'ended' && auction.highestBidderId && (
               ...
               >
                  {isFinalizing ? "Finalizing..." : "Finalize Sale"}
               </Button>
            )}
         </div>
         ...
         <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        {auction.status === "pending" && user?.role === "admin" && (
                           <div className="flex items-center gap-2 ml-auto">
                              <Button 
                                 ...
                                 className="bg-emerald-500/10 text-emerald-500 border-emerald-500/20 hover:bg-emerald-500 hover:text-white h-8 px-4 font-bold"
                              >
                              ...
                              <Button 
                                 ...
                                 disabled={isProcessingAdmin}
                                 className="bg-red-500/10 text-red-500 border-red-500/20 hover:bg-red-500 hover:text-white h-8 px-4 font-bold"
                              >
                                 <CloseIcon className="mr-1.5 h-4 w-4" /> Reject
                              </Button>
                           </div>
                        )}
            ...
            <div className="space-y-6">
               <BiddingSection 
                  auction={auction} 
                  socketData={{ highestBid, highestBidderId, isPending: isSocketPending, placeBid }}
               />
                  ...
                  <div className="flex items-center gap-4">
                     ...
                     <div>
                        ...
                        <p className="text-sm font-bold text-white">
                           {highestBidderName || (auction.highestBidderId && typeof auction.highestBidderId === 'object' ? (auction.highestBidderId as any).name : "No bids placed yet")}
                        </p>
                     </div>
                  </div>
               </Card>
            ...
            footer={
               <div className="flex gap-3">
                  ...
                  <Button 
                     ...
                     disabled={isProcessingAdmin}
                  >
                     {isProcessingAdmin ? "Processing..." : confirmModal.action === "approve" ? "Confirm Approval" : "Confirm Rejection"}
                  </Button>
               </div>
            }
         >
