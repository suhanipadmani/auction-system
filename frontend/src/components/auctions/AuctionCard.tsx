...
export function AuctionCard({ auction, href, showActions }: IAuctionCardProps) {
  ...
  useEffect(() => {
    ...
    const updateTimer = () => {
      setTimeLeft(formatTimeLeft(auction.endTime as string, t('ended')));
    };

    updateTimer();
    const interval = setInterval(updateTimer, 1000);
    return () => clearInterval(interval);
  }, [auction.endTime, auction.status, t]);
    ...
    <>
      ...
      >
        <div className="flex flex-col items-center gap-4 text-center py-4">
          ...
          <div>
            <p className="text-white font-semibold">{t('areYouSure')}</p>
            <p className="text-sm text-muted-foreground mt-1">
              {t('cancelWarning')} <span className="text-white">"{auction.title}"</span>.
            </p>
          </div>
        </div>
      </Modal>
          ...
          <div className="flex items-center gap-2">
            ...
            {isLive && (
              <Badge className="bg-emerald-500/10 text-emerald-500 border-emerald-500/20 animate-pulse flex items-center gap-1 px-1.5 py-0">
                ...
                <span className="text-[9px] font-black">{t('live')}</span>
              </Badge>
            )}
          </div>
          <div className="flex items-center gap-2">
            ...
            {auction.currentUserStatus && (
              ...
              >
                {t(`userStatus.${auction.currentUserStatus}`)}
              </Badge>
            )}
          </div>
        </CardHeader>
          ...
          <div className="grid grid-cols-2 gap-4 pt-2">
            <div className="space-y-1">
              <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">
                {auction.highestBid > 0 ? t('currentBid') : t('basePrice')}
              </span>
              <div className="flex items-center gap-1.5 text-xl font-black text-white italic">
                <span className="text-primary not-italic">{symbol}</span>
            ...
            <div className="space-y-1">
              <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest text-right block">
                {auction.status === "active" ? t('endingIn') : t('starts')}
              </span>
              <div className={cn(
                ...
                {auction.status === "active" ? timeLeft : format.dateTime(new Date(auction.startTime), { dateStyle: 'medium', timeStyle: 'short' })}
              </div>
            </div>
          </div>
        </CardContent>
